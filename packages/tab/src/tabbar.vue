<template>
    <div ref="viewArea" :class="{'lxr-tabbar':true, 'lxr-tabbar-fix-bottom': fixBottom}">
        <div :class="itemEqual?'lxr-tab-list-equal':'lxr-tab-list'" :style="style" ref="list">
            <slot></slot>
            <div class="lxr-tab-active-bar" v-if="!fixBottom"
                 :style="activeBarStyle" ref="activeBar">
            </div>
        </div>
    </div>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      // 可视区与可滑动元素宽度差值;
      listWidth () {
        return this.listOffsetWidth - this.viewAreaWidth
      },
      // 是否向左惯性滚动;
      isMoveLeft () {
    ...
    watch: {
      value () {
        this.$nextTick(() => {
          this.listOffsetWidth = this.$refs.list.offsetWidth
          this.checkPosition()
          this.calcBarPosX()
        })
      }
    ...
    methods: {
        ...
        if (this.isMoveLeft) { // 向左惯性滑动;
          if (this.translateX <= -this.listWidth) { // 超出边界的过程;
            ...
            this.acceleration *= (this.reBoundExponent +
              Math.abs(this.translateX + this.listWidth)) / this.reBoundExponent
      ...
      calcBarPosX () {
        ...
        this.activeBarWidth = Math.max(itemWidth * 1.0, 14)
...
<style>
    ...
    .lxr-tabbar.lxr-tabbar-fix-bottom {
        position: fixed;
        bottom: 0;
        left: 0;
        right: 0;
    ...
    .lxr-tabbar.lxr-tabbar-fix-bottom .lxr-tab-item {
